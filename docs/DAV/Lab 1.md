...
You can contact us on Discord or in-person during our [lab hours](https://www.ieeebruins.com/lab).
...
Your first module for the FPGA calculator will be a very basic top-level module. As we go on, we’ll add things to this module, but for now your task is to control the LEDs with the switches in a very specific way. The switches will select exactly one switch to be lit up such that inputting the number 1 (`0000000001`) will light up just the first LED from the right, the number 3 (`0000000011`) will light up the third LED from the right, and so on.
...
Now, open the Pin Planner. It should list your top-level I/O ports at the bottom below a large grid of colors and shapes. Ignore the grid and refer to the to map your pins. More detailed instructions are provided in the workshop.
...
**Checkoffs**: Show us that your testbench simulates the mini ALU (and that your mini ALU output is correct) by sending us a screenshot of your simulation waveforms on Discord.
...
The segments in each digit are represented by the picture to the left. In other words, “bit 0” (i.e. the LSB) in your 8-bit number should represent the top segment, bit 1 should represent the top-right, etc. Pay careful attention to your indexing when you assign to these segments.
...
- The first operand input to the mini ALU is the first 4 switches (as labeled in the photo above).
- The second operand input to the mini ALU is the second 4 switches (as labeled in the photo above).
- The “mode” select input to the mini ALU is the ninth switch (as labeled in the photo above).
...
**Checkoffs**: Show us your working mini ALU in person! You’ll also be expected to answer some questions about your implementation.
